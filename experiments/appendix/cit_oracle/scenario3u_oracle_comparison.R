...
# Data generation functions (from original scenario3u)
...
generate_oracle_data <- function(n, p, is_null_overall = TRUE, seed_base = NULL) {
    ...
    for (test_name in names(cit_test_functions)) {
      ...
      if (test_name == "RCIT_test") {
        ...
        result_oracle <- pbapply::pbsapply(1:n_sims, function(sim) {
          ...
          oracle_data <- generate_oracle_data(n, d, is_null_overall = is_null, seed_base = seed)
