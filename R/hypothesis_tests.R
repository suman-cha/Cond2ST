# =============================================================================
# R/hypothesis_tests.R -- Conditional two-sample test wrappers.
#
# Three families share a uniform interface:
#
#   <test_name>(x1, x2, y1, y2, ..., alpha = 0.05, seed = NULL) -> 0/1
#
#     0  : do not reject H_0 ("the conditional distributions agree")
#     1  : reject
#
#   x1, x2  -- conditioning matrices (rows = obs) for groups 1 and 2
#   y1, y2  -- response vectors for groups 1 and 2 (one-dimensional)
...
#
#   DRT (density-ratio):
#     - prop, est.prop  : sample-split fraction allocated to ratio estimation
#     - est.method      : "LL" (linear logistic) or "KLR" (kernel logistic)
#     - bandwidths      : Gaussian-kernel bandwidths for MMD variants
#     - K               : number of CV folds for cross-validated variants
#     - B               : bootstrap replication count for the quadratic MMD
#   CIT (conditional independence after Algorithm 1 subsampling):
#     - epsilon         : Algorithm 1 slack (default 1/sqrt(log n))
#     - alg1            : if FALSE, run the CIT directly without subsampling
#     - regr.method, binary.regr.method : regression closures (see R/regression.R)
#   CK (conditional-kernel direct):
#     - bandwidths set internally by the underlying KDist routines
#
# All wrappers consume their inputs as plain matrices/vectors and return a
# 0/1 indicator -- no intermediate p-values are exposed -- so that the
# Monte-Carlo runners can simply average to obtain rejection rates.
# =============================================================================

#' Debiased conditional two-sample test (Chen & Lei, 2024)
#'
#' Sample-splitting plug-in test using a debiased risk estimate. Returns
#' 0/1 (do-not-reject / reject) at level \code{alpha}.
...
DCP_test <- debiased_test
#' Density-ratio plug-in test (Chen & Lei, 2024).
#'
...
#' DRT: single-split linear MMD test.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
...
#' DRT: K-fold cross-validated linear MMD test.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
...
#' DRT: single-split block MMD test.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
BlockMMD_test <- function(x1, x2, y1, y2, B_size = NULL, prop=0.8, alpha=0.05,
...
#' DRT: K-fold cross-validated block MMD test.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
...
#' DRT: classifier-based plug-in test (single split).
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
CLF_plugin_test <- function(x1, x2, y1, y2, split.prop=0.5, est.prop=0.8, alpha=0.05, est.method="LL", seed=NULL) {
  if (!is.null(seed)) {
    set.seed(seed)
  }
  rejection <- 0
...
#' DRT: K-fold cross-validated classifier plug-in test.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
CV_CLF_plugin_test <- function(x1, x2, y1, y2, est.prop=0.8, alpha=0.05, K=2, est.method="LL", seed=NULL) {
  if (!is.null(seed)) {
    set.seed(seed)
  }
  rejection <- 0
  ...
  for (j in 1:K) {
    ...
    x12_rat <- x12[1:n1_rat, , drop = FALSE]; x12_te <- x12[-(1:n1_rat), , drop = FALSE]
    x22_rat <- x22[1:n2_rat, , drop = FALSE]; x22_te <- x22[-(1:n2_rat), , drop = FALSE]
    y12_rat <- y12[1:n1_rat]; y12_te <- y12[-(1:n1_rat)]
    y22_rat <- y22[1:n2_rat]; y22_te <- y22[-(1:n2_rat)]
  ...
  if (p_value < alpha) rejection <- 1
  return(rejection)
}
#' DRT: paired-classifier two-sample test (variant 2).
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
...
#' DRT: quadratic MMD test with parametric bootstrap.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
QuadraticMMD_test <- function(x1, x2, y1, y2, B=299, prop=0.8, alpha=0.05, bandwidths=c(1), est.method='LL', seed=NULL){
    if(!is.null(seed)){
        set.seed(seed)
    }
    ...
    for (i in seq_along(bandwidths)){
        ...
        boot_results <- quadratic_MMD(x1_test, x2_test, y1_test, y2_test,
                                      h_x=h_x, h_y=h_y, r_X=r_X2, B=B, seed=seed)

        obs_stat <- boot_results$obs_stat
        bootstrap_stats <- boot_results$bootstrap_stats

        # Calculate the p-value: include original statistic in the count
        # p = (1 + #{b : T*_b >= T_obs}) / (B + 1)
        # p_value <- (1 + sum(bootstrap_stats >= obs_stat)) / (B + 1)
        p_value <- sum(bootstrap_stats >= obs_stat) / B
...
# CV (K-fold) version of QuadraticMMD_test. Unlike the single-split
# QuadraticMMD_test, this rotates the test fold K times and averages the
# observed statistic / bootstrap distribution across folds before computing
# the p-value. Dispatches to the same quadratic_MMD() as the non-CV version,
# so it picks up the C++ backend automatically when .mmd_use_cpp is TRUE.
#' DRT: K-fold cross-validated quadratic MMD test.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
CV_QuadraticMMD_test <- function(x1, x2, y1, y2, B = 299, K = 2, prop = 0.8, alpha = 0.05,
                                 bandwidths = c(1), est.method = 'LL', seed = NULL) {
    if (!is.null(seed)) {
        set.seed(seed)
    }

    stopifnot(nrow(x1) == length(y1), nrow(x2) == length(y2), length(y1) == length(y2))
    if (K < 2) {
        stop("CV_QuadraticMMD_test requires K >= 2")
    }

    total_n <- length(y1)
    m <- total_n - ceiling(total_n * prop)
    if (m < 2) {
        stop("Each test fold needs at least two observations per group for quadratic MMD")
    }
    if (K > floor(total_n / m)) {
        stop("K must satisfy K <= floor(n / m), where m is the per-fold test size")
    }

    permuted_idx <- sample.int(total_n)
    test_folds <- vector("list", K)
    for (j in seq_len(K)) {
        start_idx <- (j - 1L) * m + 1L
        end_idx <- j * m
        test_folds[[j]] <- permuted_idx[start_idx:end_idx]
    }
    ...
    for (i in seq_along(bandwidths)) {
        ...
        fold_bootstrap <- matrix(NA_real_, nrow = K, ncol = B)

        for (j in seq_len(K)) {
            test_idx <- test_folds[[j]]
            est_idx  <- setdiff(seq_len(total_n), test_idx)
            ...
            fold_seed <- if (is.null(seed)) NULL else seed + 1000L * i + j

            ratios <- estimate_r(
                x1_est, x1_test, x2_est, x2_test,
                y1_est, y1_test, y2_est, y2_test,
                est.method = est.method, seed = fold_seed
            )
            r_X <- 1 / ratios$g22.est
            ...
            boot_results <- quadratic_MMD_regime2(
                x1_test, x2_test, y1_test, y2_test,
                h_x = h_x, h_y = h_y, r_X = r_X, B = B, seed = fold_seed
            )

            fold_stats[j] <- boot_results$obs_stat
            fold_bootstrap[j, ] <- boot_results$bootstrap_stats
        }

        obs_stat <- mean(fold_stats)
        bootstrap_stats <- colMeans(fold_bootstrap)
        p_value <- (1 + sum(bootstrap_stats >= obs_stat)) / (B + 1)
        rejections[i] <- as.integer(p_value < alpha)
    }
...
# KDist competitors (Yan et al. 2025)
# .tcdt_impl's default auto bandwidth h = n^{-1/(p/2+nu-adj_bw)} * A underflows
# in high dim because exponent < 0 forces h_j < A_j, collapsing the product
# Gaussian kernel to near-zero for d >= ~10. const_factor multiplies the auto
# formula linearly, so setting const_factor = const_factor_boot = 10 on raw X
# restores a non-degenerate kernel while keeping null near nominal.
#' CK: conditional MMD test (Park, Muandet, et al.).
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
CMMD_test <- function(x1, x2, y1, y2, alpha = 0.05, B = 199, seed = NULL,
                      cf = NULL) {
    if (is.null(cf)) cf <- as.numeric(Sys.getenv("CDTST_CMMD_CF", "10"))
    Y1 <- as.matrix(y1); Y2 <- as.matrix(y2)
    res <- KDist:::.tcdt_impl(X1 = x1, X2 = x2, Y1 = Y1, Y2 = Y2, x0 = NULL,
                              B = B, h = NULL, adj_bw = 0.1, const_factor = cf,
                              adj_bw_boot = 0, const_factor_boot = cf,
                              kern_smooth = "gaussian",
                              stat = "cmmd", kern_mmd = "gaussian",
                              sampling_method = "bootstrap", knn_k = 5, num_cores = 1)
    as.integer(res$pvalue < alpha)
}

#' CK: conditional generalised energy distance test.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
CGED_test <- function(x1, x2, y1, y2, alpha = 0.05, B = 199, seed = NULL,
                      cf = NULL) {
    if (is.null(cf)) cf <- as.numeric(Sys.getenv("CDTST_CMMD_CF", "10"))
    Y1 <- as.matrix(y1); Y2 <- as.matrix(y2)
    res <- KDist:::.tcdt_impl(X1 = x1, X2 = x2, Y1 = Y1, Y2 = Y2, x0 = NULL,
                              ...
                              adj_bw_boot = 0, const_factor_boot = cf,
                              kern_smooth = "gaussian",
                              stat = "cged", kern_mmd = "euclidean",
                              sampling_method = "bootstrap", knn_k = 5, num_cores = 1)
    as.integer(res$pvalue < alpha)
}

#' CIT: Generalised Covariance Measure with Algorithm 1 subsampling.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
...
#' CIT: Projected Covariance Measure with Algorithm 1 subsampling.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
...
#' CIT: Weighted Generalised Score Covariance with Algorithm 1 subsampling.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
...
#' CIT: Randomised CIT (Strobl et al.) with Algorithm 1 subsampling.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
...
#' CIT: Randomised Conditional Correlation Test with Algorithm 1 subsampling.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
...
#' CIT: Kernel CIT (Zhang et al.) with Algorithm 1 subsampling.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
...
#' CIT: Weighted GCM (Scheidegger et al.) with Algorithm 1 subsampling.
#'
#' @inheritParams debiased_test
#' @return Integer 0 (do not reject) or 1 (reject) at level \code{alpha}.
